...
const AdvancedAnalyticsDashboard = () => {
  ...
  // Enhanced data validation and safe defaults - moved before early returns
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                data={safeAnalyticsData.vulnerability_types}
                cx="50%"
                cy="50%"
                labelLine={false}
                ...
                dataKey="count"
              >
                {safeAnalyticsData.vulnerability_types.map((entry, index) => (
        ...
        >
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              ...
              <tbody>
                {safeAnalyticsData.top_domains.map((domain, index) => (
                  <tr key={domain.domain || index} className="border-b border-gray-100 dark:border-gray-700 last:border-0">
                    ...
                    <td className="py-3 text-sm text-gray-600 dark:text-gray-300">
                      {(domain.subdomains || 0).toLocaleString()}
                    </td>
                    <td className="py-3 text-sm">
                      <div className="flex items-center space-x-2">
                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                          (domain.vulnerabilities || 0) > 5 
                            ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'
                            : (domain.vulnerabilities || 0) > 0 
